...
public class Cows extends Canvas{
    
    CowHealth c = new CowHealth(); 
    
    BufferedImage cow = ImageIO.read(new FileInputStream("Cow.jpg"));
    BufferedImage cowL = ImageIO.read(new FileInputStream("CowL.jpg"));
       ...
       if(!dead){
           ...
           if(moveRand > 95){
               ...
               if((int)(Math.random()*2) == 0 && x < 625){
                   ...
                   moveL = false;
                   g.drawImage(cow, x, y, 25, 25, this);
               }else if(x > 0){
                   if((blockID[(x/25)-1][((1125-y)/25)] == 0 || blockID[(x/25)-1][((1125-y)/25)] == 9) || 
                   (blockID[(x/25)-1][((1125-y)/25)] != 0 && blockID[(x/25)-1][((1125-y)/25)] != 9) && 
                   (blockID[(x/25)-1][((1125-y)/25)+1] == 0 || blockID[(x/25)-1][((1125-y)/25)+1] == 9)){
                   ...
                   moveL = true;
                   g.drawImage(cowL, x, y, 25, 25, this);
               }
